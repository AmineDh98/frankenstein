...
def train_model(model, train_loader, val_loader, optimizer, scheduler, num_epochs=25, beta=1.0, pose_beta=1.0, batch_size=10):
    ...
    for epoch in range(num_epochs):
        ...
        scheduler.step(avg_val_position_loss* pose_beta + beta * avg_val_orientation_loss)

        # Early stopping check
        if early_stopping(avg_val_position_loss * pose_beta + beta * avg_val_orientation_loss):
